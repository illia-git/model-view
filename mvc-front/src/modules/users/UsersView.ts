...
import { TSortField, TSortOrder, IUser } from './UsersModel';
...
export class UsersView {
	...
	private static getUserElement(user : IUser) {
		return `
                 <div class="user">
	...
	private onCreateClick = () => {
		try {
			const newUser = this.controller.handleCreate(this.usernameInput.value,
				Number(this.ageInput.value));
			this.renderNewUser(newUser);
		} catch (err) {
			UsersView.showError((err as Error).message);
		}
	};

	private onSortClick = () => {
		const newUsers = this.controller.handleSort(this.fieldSelect.value as TSortField,
			this.orderSelect.value as TSortOrder);
	...
	private renderNewUser(user : IUser) {
		const userNode = document.createElement('div');
		userNode.innerHTML = UsersView.getUserElement(user);

		this.users.appendChild(userNode);
	}

	private renderUsers(users : IUser[]) {
		const usersElements = users.map(user => {
			return UsersView.getUserElement(user);
		});
	...
	private createSortSelectors() {
		...
		usernameOption.innerText = 'Username';
		...
		ascOption.value = 'asc';
		ascOption.innerText = 'Asc';
		const descOption = document.createElement('option');
		descOption.value = 'desc';
		descOption.innerText = 'Desc';
	...
	private createUserForm() {
		...
		this.usernameInput.placeholder = 'Paste username';
		this.ageInput = document.createElement('input');
		this.ageInput.placeholder = 'Paste age';
